...
void single_leg_pattern(mjData *d) {
  ...
  if(dt<=theta && theta<=2*dt){
    ...
    d->ctrl[17] = clip(sin(theta - 0.7) * -target_foot, -15.0, -10.0); // bl_3_act
  }
  ...
  //   d->ctrl[15] = sin(theta+ M_PI) * target_hip;                             // bl_1_act
  //   d->ctrl[16] = clip(sin(theta + M_PI - 0.7) * -target_knee, -2.0, 4.0);    // fr_2_act
  //   d->ctrl[17] = clip(sin(theta + M_PI - 0.7) * -target_foot, -15.0, -10.0); //fr_3_act
  // }
  // if(M_PI <= theta && theta <= 2.0 * M_PI){
  //   d->ctrl[5] = sin(theta + 2.0*M_PI ) * -target_hip;                          // fl_1_act
  //   d->ctrl[6] = clip(sin(theta + 2.0*M_PI  - 0.7) * target_knee, -7.0, 0.0);   // fl_2_act
  //   d->ctrl[7] = clip(sin(theta + 2.0*M_PI  - 0.7) * -target_foot, -15.0, 0.0); // fl_2_act
  // }
  // //
  // if(M_PI <= theta && theta <= 3.0 * M_PI){
  //   d->ctrl[10] = sin(theta + 3.0* M_PI) * -target_hip;                          // br_1_act
  //   d->ctrl[11] = clip(sin(theta + 3.0* M_PI - 0.7) * target_knee, -7.0, 0.0);   // br_2_act
  //   d->ctrl[12] = clip(sin(theta + 3.0* M_PI - 0.7) * -target_foot, -15.0, 0.0); // br_2_act
  // }
  // printf("clipped: %f\n", d->ctrl[12]);                              // fr_2_act
