...
/*** Flying-Dog ***/
void renderActuatorForces(mjModel *m, mjData *d, mjvOption *opt,
                          mjvPerturb *pert, mjvCamera *cam, mjvScene *scn) {
  // const char *actuator_names[16] = {
  //     "fr_1_act", "fr_2_act", "fr_3_act", "fr_prop",  "fl_1_act", "fl_2_act",
  //     "fl_3_act", "fl_prop",  "br_1_act", "br_2_act", "br_3_act", "br_prop",
  //     "bl_1_act", "bl_2_act", "bl_3_act", "bl_prop"};
  const int num_render_locations = 4;
  const char *actuator_names[num_render_locations] = {"fr_prop_motor", "fl_prop_motor",
                                                      "br_prop_motor", "bl_prop_motor"};
  const char *thruster_names[num_render_locations] = {"fr_s1", "fl_s1", "br_s1",
                                                      "bl_s1"};

  for (int i = 0; i < num_render_locations; i++) {
...
/*** Flying-Dog ***/
void ShowSubCAM(mj::Simulate *sim, mjrRect rect, mjvScene *scn, mjvCamera cam,
                mjvOption *opt, mjvPerturb *pert) {
  // Refer:
  // https://github.com/dtorre38/mujoco_opencv/blob/fa9e61fcf5fa3f7048c4aac08dde4f3b37ea1f12/utils/render_insetwindow.c#L6
  mjrRect viewport = {rect.left + rect.width - rect.width / 3, rect.bottom,
  ...
  for (int i = 0;
       i < (VIEWPORT_HEIGHT * VIEWPORT_WIDTH * 3 * sizeof(unsigned char));
       i++) {
    _avData->color_buffer[i] = sim->avData->color_buffer[i]; // copy data
  }
  mjr_readPixels2(_avData->color_buffer, nullptr, viewport,
                  &sim->platform_ui->mjr_context(), 360, 270);

  (*image_ptr).data = _avData->color_buffer;
  // float pts[8] = {0};
  // int npts = auvc::processImage(*image_ptr, *image_gray_ptr, *flipped_ptr,
  // pts, m_tagDetector); printf("npts: %d\n", npts);
...
/*** Flying-Dog ***/
bool Simulate::reloadControllerLibPlug(mjModel *m, mjData *d, bool test,
                                       bool load_dll) {
  ...
  if (libcontroller == NULL) {
    fprintf(stderr, "ERROR: (%d) could not load %s: %s\n", __LINE__,
            libcontroller_filename, dlerror());
    return false;
  }
  plug_controller_test =
      (plug_test_t)dlsym(libcontroller, "controllerTestFunc");
  if (plug_controller_test == NULL) {
    fprintf(stderr,
            "ERROR: (%d) could not load symbol test_func() from %s: %s\n",
  ...
  plug_controller_init =
      (plug_init_t)dlsym(libcontroller, "controllerInitPlug");
  if (plug_controller_init == NULL) {
    fprintf(stderr,
            "ERROR: (%d) could not load symbol initPlug() from %s: %s\n",
            __LINE__, libcontroller_filename, dlerror());
    dlclose(libcontroller);
    return false;
  }
  ...
  plug_controller_update =
      (plug_update_t)dlsym(libcontroller, "controllerUpdatePlug");
  if (plug_controller_update == NULL) {
    fprintf(stderr,
            "ERROR: (%d) could not load symbol updatePlug() from %s: %s\n",
            __LINE__, libcontroller_filename, dlerror());
    dlclose(libcontroller);
    return false;
  }
  ...
  if (load_dll) {
    ...
    cond_reload_plug_bool.wait(
        lock, [this]() { return this->reload_plug_bool == 0; });
...
/*** Flying-Dog ***/
bool Simulate::reloadPhysicsLibPlug(mjModel *m, mjData *d, bool test,
                                    bool load_dll) {
  if (libphysics != NULL) {
    dlclose(libphysics);
  }
  ...
  if (test) {
    plug_physics_init(m, d);
  }
  plug_physics_update =
      (plug_update_t)dlsym(libphysics, "roverPhysicsUpdatePlug");
...
void Simulate::RenderLoop(auvcData *avD) {
  ...
  auvcData av_newData;
  _avData = &av_newData;
  _avData->color_buffer = (unsigned char *)malloc(
      VIEWPORT_HEIGHT * VIEWPORT_WIDTH * 3 * sizeof(unsigned char));
